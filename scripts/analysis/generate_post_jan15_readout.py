...
def get_post_jan15_decisions():
    """Fetch decision points from the database after Jan 15, 2026."""
...
def generate_readout():
    print("Fetching decision data from database (post Jan 15, 2026)...")
    df = get_post_jan15_decisions()
    
    # Exclude basic AVOID and PENDING if needed
    # Including basic AVOID as requested by user earlier
    # BUT exclude PENDING as per new request
    df = df[df['recommendation'] != 'PENDING']
    print(f"Analyzing {len(df)} recommendations (excluding PENDING)...")
    
    # Pre-fetch S&P 500 history for 2026 onwards to avoid fetching it for every symbol
    print("Fetching S&P 500 baseline data...")
    sp500_history = yf.Ticker("^GSPC").history(start="2026-01-15")
    ...
    # To include all data, don't filter for >= 3 so we get an idea of all trades, but let's keep it for visual clarity
    # Since post-Jan 15 might have fewer trades, we'll decrease min count to 1
    ...
    plot_df = perf_df[perf_df.groupby('Cohort')['Cohort'].transform('size') >= 1]
    
    # Use stripplot since N might be small in violinplot
    if len(plot_df) >= 3:
        try:
            sns.violinplot(
                data=plot_df, 
            ...
            sns.boxplot(data=plot_df, x='max_roi', y='Cohort')
    else:
        sns.barplot(data=plot_df, x='max_roi', y='Cohort')

    plt.axvline(x=10, color='r', linestyle='--', label='10% Target Win')
    plt.title("Distribution of Max ROI by Recommendation Cohort (Post Jan 15th)")
    plt.xlabel("Maximum ROI (%)")
    plt.ylabel("Recommendation + Deep Research Verdict")
    plt.legend()
    plt.tight_layout()
    
    plot_path = "reports/post_jan15_roi_distribution.png"
    ...
    plt.title("Win Rate (>10% Max ROI) by Recommendation Cohort (Post Jan 15th)")
    plt.xlabel("Win Rate (%)")
    plt.ylabel("Recommendation + Deep Research Verdict")
    plt.tight_layout()
    
    win_rate_plot_path = "reports/post_jan15_win_rate_distribution.png"
    ...
    plt.title("Average Max ROI vs SP500 Average Max ROI by Cohort (Post Jan 15th)")
    plt.xlabel("Average ROI (%)")
    plt.ylabel("Recommendation + Deep Research Verdict")
    plt.legend(title='Metric')
    plt.tight_layout()
    
    roi_comparison_plot_path = "reports/post_jan15_roi_comparison.png"
    ...
    report_content = [
        "# Post-Jan 15th 2026 Trading Performance Readout\n",
        "This report analyzes the performance of all system recommendations (excluding generic AVOID) made after January 15th, 2026.\n",
        "**Metrics Defined:**",
    ...
    for _, row in grouped.iterrows():
        ...
        avg_date_str = row['avg_timestamp'].strftime('%b %d') if pd.notnull(row['avg_timestamp']) else "N/A"
        
        report_content.append(
            f"| {row['recommendation']} | {row['deep_research_verdict']} | {row['count']} | {avg_date_str} | "
            f"{row['avg_max_roi']:.2f}% | {sp500_roi_str} | {row['trigger_rate']:.1f}% | "
            f"{limit_roi_str} | {row['win_rate']:.1f}% | {row['avg_current_roi']:.2f}% |"
        )
        
    report_content.append(f"\n## Visual Analysis")
    report_content.append(f"\n### Distribution of Max ROI")
    report_content.append(f"![ROI Distribution](post_jan15_roi_distribution.png)\n")
    
    report_content.append(f"\n### Win Rate by Cohort")
    report_content.append(f"![Win Rate Distribution](post_jan15_win_rate_distribution.png)\n")
    
    report_content.append(f"\n### Stock ROI vs S&P 500 Baseline")
    report_content.append(f"![ROI Comparison](post_jan15_roi_comparison.png)\n")
    ...
    report_path = "reports/post_jan15_trading_readout.md"
